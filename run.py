...
if __name__ == '__main__':
    ...
    parser.add_argument('--band_attention_activation', type=str, default='identity_softmax', choices=['identity_softmax', 'tanh', 'softmax', 'sigmoid'], help='Band attention normalization used to convert band logits into importance weights')
    parser.add_argument('--band_attention_pooling', type=str, default='energy', choices=['energy', 'abs', 'mean'], help='Statistic used to pool SWT coefficients into band descriptors')
    parser.add_argument('--band_attention_per_variable', action='store_true', help='Learn a separate band profile for each variable')
    parser.add_argument('--band_attention_apply_to', type=str, default='v', choices=['v', 'qk', 'qkv'], help='SWT streams that receive band weights')
    parser.add_argument('--band_attention_separate_qkv', action='store_true', help='Compute separate Q, K and V band profiles instead of sharing Q weights')
    parser.add_argument('--band_attention_horizon', action='store_true', help='Condition band profiles on log prediction horizon')
    parser.add_argument('--use_band_mixing', action='store_true', help='Enable residual coarse-to-fine SWT band mixing')
    parser.add_argument('--band_mixing_hidden_dim', type=int, default=None, help='Hidden dimension of the band mixing MLP')
    parser.add_argument('--band_mixing_dropout', type=float, default=0.0, help='Dropout probability within band mixing')
    parser.add_argument('--band_mixing_unidirectional', action='store_true', help='Use coarse-to-fine band mixing only')
